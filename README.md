...
- “预计支付账户”优先采用钉钉的“预计支付账户 / Cuenta de pago prevista”；未填写时按“服务主体 / Sujeto de servicio”生成公司账户默认值。当前默认映射为悦为智能、凌翔、星铭、拉丁购和 YW MOLDES 五个公司账户，无法识别的主体保持为空并提示人工填写。
- 预计支付账户的来源由系统内部记录。网页或 Excel 的人工填写结果不会被后续钉钉同步覆盖；人工清空后，下一次同步可以重新采用钉钉明确值或服务主体默认值。
...
- “预计支付账户”默认显示在“账户性质”之后，可在列设置中隐藏；请款抽屉、手机卡片及工作台 Excel 导入导出均保留该字段。
...
- 墨西哥审批与中国工作台、每日应付的 Sheet 权限相互独立；用户范围分为“全部审批”“仅本人参与”和“不可访问”。
- “仅本人参与”按绑定的钉钉姓名精确匹配申请人、当前审批人、历史操作人和抄送人，列表、统计、详情及附件使用同一权限边界。
- 页面可在“审批列表”和“审批人统计”之间切换；点击审批人会返回列表并自动筛选其待办，黄色/红色预警可继续与人名组合过滤。
- 管理员在用户管理中维护墨西哥权限及钉钉审批姓名；批量开通确认账号使用 `python -m backend.app.provision_mexico_users --actor-username admin`。
- 生产发布、验收和回滚步骤见 [`deploy/mexico-approver-access-release.md`](deploy/mexico-approver-access-release.md)。

预计支付账户字段的生产迁移、同步验收和回滚步骤见 [`deploy/expected-payment-account-release.md`](deploy/expected-payment-account-release.md)。
